...
/// Main translator screen for WorldTalk AI.
...
class TranslatorScreen extends ConsumerStatefulWidget {
  ...
  const TranslatorScreen({super.key, this.initialSourceLang, this.initialTargetLang,});

  @override
  ConsumerState<TranslatorScreen> createState() => _TranslatorScreenState();
}

class _TranslatorScreenState extends ConsumerState<TranslatorScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (widget.initialSourceLang != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        final lang = kSupportedLanguages.firstWhere((l) => l.code == widget.initialSourceLang, orElse: () => kSupportedLanguages.first);
        ref.read(languageProvider.notifier).setSourceLanguage(lang);
      });
    }
    if (widget.initialTargetLang != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        final lang = kSupportedLanguages.firstWhere((l) => l.code == widget.initialTargetLang, orElse: () => kSupportedLanguages.first);
  ...
  @override
  void dispose() { _inputCtrl.dispose(); _debounceTimer?.cancel(); super.dispose(); }
  ...
  Future<void> _translate() async {
    ...
    try {
      final sourceLang = langState.autoDetect ? 'auto' : langState.sourceLanguage.code;
      final result = await service.translateText(text: text, sourceLang: sourceLang, targetLang: langState.targetLanguage.code);
      setState(() { _translatedText = result; _hasTranslated = true; });
      await HistoryService.saveTranslation(TranslationResult(id: _uuid.v4(), sourceText: text, translatedText: result, sourceLang: sourceLang, targetLang: langState.targetLanguage.code, timestamp: DateTime.now()));
    } on TranslationException catch (e) { ref.read(languageProvider.notifier).setError(e.message); }
    catch (e) { ref.read(languageProvider.notifier).setError('Something went wrong.'); } finally { ref.read(languageProvider.notifier).setTranslating(false); }
  }
  ...
  void _swapLanguages() {
    ...
    if (_hasTranslated) {
      final original = _inputCtrl.text;
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_inputCtrl.text.isNotEmpty || _hasTranslated)
            IconButton(icon: const Icon(Icons.close, size: 20), tooltip: 'Clear', onPressed: _clearAll),
        ],
      ),
      body: SafeArea(
        child: Column(children: [
          ...
          if (ls.errorMessage != null)
            Container(
              ...
              decoration: BoxDecoration(color: AppColors.coral.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.coral.withValues(alpha: 0.3))),
          ...
          Expanded(
            child: Column(children: [
              Expanded(flex: 5, child: TranslationInputField(controller: _inputCtrl, hintText: 'Type or paste text to translate...', onChanged: _onTextChanged, onClear: _inputCtrl.text.isNotEmpty ? () => _inputCtrl.clear() : null)),
              const Divider(height: 1),
              Expanded(flex: 5, child: TranslationOutputPanel(text: _translatedText, isLoading: ls.isTranslating, hasInput: _inputCtrl.text.isNotEmpty, onCopy: _copyOutput, onShare: _hasTranslated ? () {} : null, onRetry: ls.errorMessage != null ? _translate : null)),
            ]),
          ),
          _BottomActionBar(
            hasInput: _inputCtrl.text.isNotEmpty,
            hasOutput: _hasTranslated,
            isTranslating: ls.isTranslating,
...
class _BottomActionBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      if (hasInput || hasOutput) _IconActionBttton(icon: Icons.close, tooltip: 'Clear all', onTap: onClear),
      const Spacer(),
      ElevatedButton.icon(
        onPressed: isTranslating || !hasInput ? null : onTranslate,
        icon: isTranslating ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: AppColors.ink)) : const Icon(Icons.translate, size: 18),
      ...
      const Spacer(),
      if (hasOutput) _IconActionBttton(icon: Icons.copy, tooltip: 'Copy', onTap: onCopy),
    ]),
  );
}

class _IconActionBttton extends StatelessWidget {
  final IconData icon;
  final String tooltip;
  final VoidCallback onTap;
  const _IconActionBttton({required this.icon, required this.tooltip, required this.onTap});
  @override
